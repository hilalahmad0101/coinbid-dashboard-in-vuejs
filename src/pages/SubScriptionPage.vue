            ...
            <table class="w-full">
              <thead class="bg-[#D7D8DB] px-3 py-4">
                <tr>
                  ...
                  <td class="text-left px-6 py-5">Total Ads</td>
              ...
              <tbody>
                <tr v-for="(pk, index) in packages" :key="index" class="border-b">
                  ...
                  <td
                    class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap"
                  >{{pk.expire_date <= 0 ?'Package Expired': pk.expire_date+ ' Days is Remaining '}}</td>
                  <td
                    class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap"
                  >{{pk.coins}}</td>
                  <!-- <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap">
                    <img
                      :src="'http://localhost:500/uploads/'+pk.icon"
                      class="w-12 h-12"
                      alt
                    />
                  </td>-->
...
export default {
  ...
  methods: {
    ...
    async getPackages() {
      ...
      this.packages.map(val => {
        var given = moment(val.expire_date, "YYYY-MM-DD");
        var current = moment().startOf("day");
        //this.totalDays = moment.duration(given.diff(current)).asDays();
        val.new_expire_date = moment.duration(given.diff(current)).asDays();
        //console.log(this.totalDays);
        // console.log(val.new_expire_date);
      });
      // console.log("AFter Map");
      // console.log(this.packages);
      //this.totalDays = `<span>${this.totalDays}</span>`;
    },
    async deletePackage(id) {
      if (confirm("Are you sure you want to delete")) {
        const res = await (
          await fetch(`${BASE_URL}admin/package/plan/${id}`, {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
              admin_access_token: localStorage.getItem("token")
