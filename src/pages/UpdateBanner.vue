...
export default {
  ...
  methods: {
    ...
    async getData() {
      const res = await (
        await fetch(`${BASE_URL}admin/banner/${this.$route.params.id}`, {
          method: "PATCH",
          headers: {
      ...
      if (res.success) {
        this.showImage = res.banners.image;
        this.title = res.banners.title;
        this.color = res.banners.color;
      }
    },
    async updateBanner() {
      ...
      const res = await axios.put(
        `${BASE_URL}admin/banner/${this.$route.params.id}`,
        formdata,
        {
          headers: {
            "Content-Type": "application/json",
