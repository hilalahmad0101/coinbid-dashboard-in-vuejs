<template>
  <div class="flex">
    ...
    <div class="h-full w-full to-[#F5F5F5] from-white bg-gradient-to-r lg:ml-[300px]">
      <Navbar @sideBar="showNavBar" title="Transaction History" />
      <div
        class="bg-white rounded-md shadow mx-10 my-5"
        :class="transactions.length > 0 ? 'p-0':'p-10'"
      >
              ...
              <table class="w-full">
                <thead class="bg-[#D7D8DB] px-3 py-4">
                  <tr>
                    ...
                    <td class="text-left px-6 py-5">Username</td>
                    <td class="text-left px-6 py-5">Transaction</td>
                    <!-- <td class="text-left px-6 py-5">Action</td> -->
                  </tr>
                </thead>
                <tbody v-for="(transaction, index) in transactions" :key="index" class="border-b">
                  <tr>
                    <td
                    ...
                    <td
                      class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap"
                    >{{transaction.users == null ? 'User Deleted':transaction.users.name}}</td>
                    <td
                      class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap"
...
export default {
  ...
  methods: {
    ...
    async getAds() {
      this.isLoading = true;
      const res = await (
        await fetch(`${BASE_URL}admins/transactions`, {
          method: "GET",
          headers: {
