            ...
            <table class="w-full">
              <thead class="bg-[#D7D8DB] px-3 py-4">
                <tr>
                  ...
                  <td class="text-left px-6 py-5">Tax</td>
                  <td class="text-left px-6 py-5">Action</td>
                </tr>
              </thead>
              <tbody v-for="(tax, index) in taxs" :key="index" class="border-b">
                <tr>
                  <td
                  ...
                  <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap">
                    <router-link :to="'/update/taxs/'+tax._id">
                      <button>
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
...
export default {
  ...
  methods: {
    ...
    async getTax() {
      this.isLoading = true;
      const res = await (
        await fetch(`${BASE_URL}admin/tax`, {
          method: "GET",
          headers: {
    ...
    //   const res = await (
    //     await fetch(`${BASE_URL}admin/bounes/coin/${id}`, {
    //       method: "DELETE",
    //       headers: {
    //         "Content-Type": "application/json",
    //         admin_access_token: localStorage.getItem("token")
