<template>
  <div class="flex">
    ...
    <div class="h-full w-full to-[#F5F5F5] from-white bg-gradient-to-r lg:ml-[300px]">
      ...
      <div class="bg-white rounded-md shadow mx-10 my-5" :class="ads.length > 0 ? 'p-0':'p-10'">
                  ...
                  <tr>
                    ...
                    <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap flex">
                      <span
                        ...
                        class="space-x-5"
                      >
                        <img :src="`${path}uploads/${item}`" class="h-10 w-10 ml-2" alt />
                      </span>
                    </td>
                    ...
                    <!-- <td
                      ...
                      v-if="ad.video"
                    >
                      <video :src="`${path}uploads/${ad.video}`" controls class="w-10 h-10"></video>
                    </td>
                    <td v-else>Video not have</td>-->
...
export default {
  ...
  methods: {
    ...
    async getAds() {
      this.isLoading = true;
      const res = await (
        await fetch(`${BASE_URL}admin/ads`, {
          method: "GET",
          headers: {
    ...
    async deleteAds(id) {
      const res = await (
        await fetch(`${BASE_URL}admin/ads/${id}`, {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
            admin_access_token: localStorage.getItem("token")
